...
template: "post"
draft: true
slug: "/posts/understanding-virtual-dom-react/"
category: "react"
tags:
  - "React"
  - "가상 DOM"
...
다시 처음으로 돌아가서, 이번엔 (바닐라) 자바스크립트와 리액트를 비교해보자.
...
<html>
  ...
  <body>
    ...
    <script>
      ...
      $app.innerHTML += Avatar({ id: 75, name: "Gale" });
      $app.innerHTML += Avatar({ id: 76, name: "Vincent" });
      const newArr = Array.from(Array(10).keys());
      newArr.forEach((el) => ($app.innerHTML += Avatar({ id: el })));
    </script>
  </body>
</html>
```

위 코드는 랜덤 유저들의 사진을 나열하는 단순한 웹 페이지이다.

JSX 없는 리액트로 작성하면 아래 ㅗㅋ드와 같다.
